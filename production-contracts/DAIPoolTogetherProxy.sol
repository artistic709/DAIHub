...
contract DAIPoolTogetherProxy is Ownable {
    using SafeMath for uint256;

    PoolTogether constant poolTogether = PoolTogether(0x29fe7D60DdF151E5b52e5FAB4f1325da6b2bD958);
    ...
    constructor(address _wallet, uint256 _reserveRate) public {
        DAI.approve(address(poolTogether), uint256(-1));
        wallet = _wallet;
    ...
    function deposit(uint256 amount) external onlyHub {
        ...
        poolTogether.depositPool(amount);
    }

    function withdraw(address to, uint256 amount) external onlyHub {
        totalValueStored = updateTotalValue().sub(amount);

        uint256 depositBalance = poolTogether.openBalanceOf(address(this));
        if(amount <= depositBalance) {
            poolTogether.withdrawOpenDeposit(amount);
        }
        else {
            poolTogether.withdrawOpenDeposit(depositBalance);
            poolTogether.withdrawCommittedDeposit(amount - depositBalance);
        }
    ...
    function updateTotalValue() internal returns(uint256) {
        uint256 newBalance = poolTogether.totalBalanceOf(address(this));
