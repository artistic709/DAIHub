...
contract DAIIearnProxy is Ownable {
    using SafeMath for uint256;

    IERC20 constant YDAI = IERC20(0x16de59092dAE5CcF4A1E6439D611fd0653f0Bd01);
    ...
    constructor(address _wallet, uint256 _reserveRate) public {
        DAI.approve(address(YDAI), uint256(-1));
        wallet = _wallet;
    ...
    function withdraw(address to, uint256 amount) external onlyHub {
        ...
        uint256 burnAmount = amount.mul(1e18).div(YDAI.getPricePerFullShare());
        burnAmount = burnAmount < YDAI.balanceOf(address(this)) ? burnAmount.add(1) : burnAmount;
        require(YDAI.withdraw(burnAmount));
    ...
    function updateTotalValue() internal returns(uint256) {
        uint256 newBalance = YDAI.BalanceOf(address(this)).mul(YDAI.getPricePerFullShare());
